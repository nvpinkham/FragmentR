...
find.match3 <- function(file.query, ladder = rox.ladder,
                        database = cd.db, meth= "bc", log.trans = F){

  bp.query <- sum.fsa(file.query, ladder, cutoff = database$settings[1], sd = database$settings[2])
...
make.database <- function(folder = "F-RibotypingFiles", ladder,
  ...
  db.args <- c(cutoff, sd, bp.min, bp.max)
  names(db.args) <-  c("cutoff", "sd", "bp.min", "bp.max")
  db.final <- list(db.filtered, db.args, ladder)
